...
def validate_top_nav_menu():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == locators.aos_homepage_title:
        driver1.find_element(By.XPATH, '//span[contains(.,"dvantage")]').is_displayed()
        sleep(0.25)
        print(f'Main logo is displayed!')
        driver1.find_element(By.XPATH, '//a[contains(text(),"OUR PRODUCTS")]').is_displayed()
        sleep(0.25)
        print(f'OUR PRODUCTS is displayed!')
        driver1.find_element(By.XPATH, '//a[contains(text(),"SPECIAL OFFER")]').is_displayed()
        sleep(0.25)
        print(f'SPECIAL OFFER is displayed!')
        driver1.find_element(By.XPATH, '//a[contains(text(),"POPULAR ITEMS")]').is_displayed()
        sleep(0.25)
        print(f'POPULAR ITEMS is displayed!')
        driver1.find_element(By.XPATH, '//a[contains(text(),"CONTACT US")]').is_displayed()
        sleep(0.25)
...
def validate_homepage_texts_links():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == locators.aos_homepage_title:
        driver1.find_element(By.ID, 'speakersTxt').is_displayed()
        sleep(0.25)
        driver1.find_element(By.ID, 'tabletsTxt').is_displayed()
        sleep(0.25)
        driver1.find_element(By.ID, 'laptopsTxt').is_displayed()
        sleep(0.25)
        driver1.find_element(By.ID, 'miceTxt').is_displayed()
        sleep(0.25)
        driver1.find_element(By.ID, 'headphonesTxt').is_displayed()
        sleep(0.25)
...
def check_social_network_facebook():
    ...
    if driver1.find_element(By.XPATH, '//h3[contains(., "FOLLOW US")]').is_displayed():
        print('We could find FOLLOW US displayed')
        sleep(1)
        facebook = driver1.find_element(By.XPATH, '//img[@name="follow_facebook"]')
        ...
        driver1.find_element(By.XPATH, '//img[@name="follow_facebook"]').click()
...
def validate_contact_us_form():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == locators.aos_homepage_title:
        ...
        display_btn = driver1.find_element(By.XPATH, '//a[@class="a-button ng-binding"]')
        sleep(2)
        print(f'the CONTINUE SHOPPING button display feature is : {display_btn.is_displayed()}')
        sleep(1)
        driver1.find_element(By.XPATH, '//a[@class="a-button ng-binding"]').click()
        sleep(2)
...
def create_new_account():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == locators.aos_homepage_title:
        ...
        sleep(2)
    if driver1.current_url == locators.aos_register_url or driver1.title == aos_homepage_title:
...
def validate_user_login():
    ...
    if driver1.current_url == locators.aos_url and driver1.title == locators.aos_homepage_title:
        if driver1.find_element(By.XPATH, f'//a[contains(.,"{locators.new_username}")]'):
            sleep(3)
            print(
...
def checkout_shopping_cart():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == aos_homepage_title:
        driver1.find_element(By.ID, 'speakersTxt').click()  # Click SPEAKERS
        sleep(2)
        if driver1.current_url == 'https://advantageonlineshopping.com/#/category/Speakers/4':
            # navigate to SPEAKER page and click Bose Soundlink wireless speaker
            driver1.find_element(By.ID, '25').click()
            sleep(2)
        if driver1.current_url == 'https://advantageonlineshopping.com/#/product/25':
            driver1.find_element(By.NAME, 'save_to_cart').click()
            sleep(2)
            driver1.find_element(By.ID, 'shoppingCartLink').click()
            sleep(2)
            print('You have successfully added one item in the shopping cart!')
            # check out shopping cart
            driver1.find_element(By.ID, 'checkOutButton').click()   # Click check out
            ...
            driver1.find_element(By.ID, 'next_btn').click()
            sleep(5)
            if driver1.current_url == locators.aos_order_payment_url:
                ...
                driver1.find_element(By.NAME, 'safepay_username').send_keys(locators.safepay_username)    # safepay
                sleep(0.25)
                driver1.find_element(By.NAME, 'safepay_password').send_keys(locators.safepay_password)
                sleep(0.25)
                driver1.find_element(By.NAME, 'save_safepay')
                sleep(1)
                driver1.find_element(By.ID, 'pay_now_btn_SAFEPAY').click()
                sleep(2)
...
def validate_order():
    ...
    if driver1.current_url == locators.aos_order_payment_url:
        ...
        sleep(1)
        driver1.find_element(By.ID, 'trackingNumberLabel').is_displayed()
        sleep(2)
        print(f'Tracking number was captured {locators.tracking_number}')
        driver1.find_element(By.ID, 'orderNumberLabel').is_displayed()
        sleep(2)
...
def log_in():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == aos_homepage_title:
        ...
        # Access user's 'My Order' account
            driver1.find_element(By.ID, "menuUserLink").click()
            sleep(2)
            driver1.find_element(By.XPATH, '//*[@id="loginMiniTitle"]/label[2]').click()
            sleep(2)
            locators.order_element = driver1.find_element(By.XPATH, f'//label[contains(., "{locators.order_number}")]')
            print(f'The order number is matching with the one that captured  on the screen : {locators.order_element}')
            sleep(2)
            driver1.find_element(By.LINK_TEXT, 'REMOVE').click()
            sleep(2)
            print(f'Are you sure you want to remove the order?')
            driver1.find_element(By.XPATH, f'//label[contains(., ", CANCEL")]').click()
            sleep(2)
            print(f'Yes! CANCEL')
            driver1.find_element(By.ID, "menuUserLink").click()
            sleep(2)
...
def delete_account():
    ...
    if driver1.current_url == locators.aos_orders_url:
        driver1.find_element(By.ID, 'menuUserLink').click()
        sleep(2)
        driver1.find_element(By.XPATH, '//*[@id="loginMiniTitle"]/label[1]').click()
        sleep(2)
        # validate full name and click delete button
        name = driver1.find_element(By.XPATH, f'//label[contains(.,"{locators.full_name}")]')
        print(f'Your account is validated: {name.is_displayed()} and name is : {locators.full_name}!')
        driver1.find_element(By.CLASS_NAME, 'deleteBtnText').click()
        sleep(2)
        driver1.find_element(By.ID, 'deleteAccountPopup').is_displayed()
        print(f'Are sure you want to delete account?')
        sleep(2)
        driver1.find_element(By.XPATH, '//*[@id="deleteAccountPopup"]/div[3]/div[1]').click()
        sleep(2)
        print(f'YES')
...
def validate_user_deleted():
    ...
    if driver1.current_url == locators.aos_url or driver1.title == aos_homepage_title:
        ...
        driver1.find_element(By.ID, 'sign_in_btnundefined').click()
        sleep(2)
        driver1.find_element(By.ID, 'signInResultMessage').is_displayed()
        sleep(2)
